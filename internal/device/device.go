...
func (d *Device) GetConfigFromFile(filename string) {
	...
	log.Printf("trigger on %s", InternalSensor+d.DeviceID+TriggerSuffix)
}
...
func (d *Device) Listen() {
	...
	d.MQTT.Subscribe(InternalSensor + d.DeviceID + TriggerSuffix)
	//d.sendStatus()
...
func (d *Device) action(m mqtt.Message, client mqtt.Client, channel string) {
	...
	} else if channel == InternalSensor+d.DeviceID+TriggerSuffix {
		log.Printf("%s, %s", m.Payload(), m.Topic())
		compo := strings.Split(m.Topic(), "/")
		log.Printf("%+v", compo)
		d.sendTwinSensorActualValue(compo[2], string(m.Payload()))
	}
}
...
func (d *Device) sendTwinSensorActualValue(device, value string) {
	...
	actualMap := map[string]*dttype.MsgTwin{
		device: {Actual: &dttype.TwinValue{Value: &value}, Metadata: &dttype.TypeMetadata{Type: "Updated"}},
	}
	...
	log.Print(string(twinUpdateBody))
